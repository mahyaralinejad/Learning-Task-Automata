...
grid_size = 4;
P_chain = zeros(grid_size^2,grid_size^2);

for s1=1:grid_size
    for s2=1:grid_size
        if s1 ~= grid_size
            P_chain(grid_coord(s1,s2,grid_size), grid_coord(s1+1,s2,grid_size)) = 0.25;
        else
            P_chain(grid_coord(s1,s2,grid_size), grid_coord(s1,s2,grid_size)) = P_chain(grid_coord(s1,s2,grid_size), grid_coord(s1,s2,grid_size)) + 0.25;
        end
        if s2 ~= grid_size
            P_chain(grid_coord(s1,s2,grid_size), grid_coord(s1,s2+1,grid_size)) = 0.25;
        else
            P_chain(grid_coord(s1,s2,grid_size), grid_coord(s1,s2,grid_size)) = P_chain(grid_coord(s1,s2,grid_size), grid_coord(s1,s2,grid_size)) + 0.25;
        end
        if s1 ~= 1
...
labels = [ 
    ...
    %3,3,7;
    2,grid_size,3; % tv
    grid_size,2,6; % stairs
    grid_size,grid_size,5
    ];

for i=1:size(labels, 1)
    L(grid_coord(labels(i,1), labels(i,2), grid_size)) = labels(i,3); 
end
...
wins = observationSequences > grid_size^2;
...
its = 25;
for i=1:100
    [T_est, E_est] = hmmtrain(observationSequences, T_est, E_est, 'Tolerance', 1e-6, 'Maxiterations',its);%,'Verbose',true);
    disp(T_est)
    disp(i*its)
    save(string(grid_size)+"x"+string(grid_size)+"case_study_"+string(round(n_seed)))
    timeVal = toc
end