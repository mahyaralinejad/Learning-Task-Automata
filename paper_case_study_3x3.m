...
for s1=1:3
    for s2=1:3
        if s1 ~= 3
            P_chain(grid_coord(s1,s2,3), grid_coord(s1+1,s2,3)) = 0.25;
        else
            P_chain(grid_coord(s1,s2,3), grid_coord(s1,s2,3)) = P_chain(grid_coord(s1,s2,3), grid_coord(s1,s2,3)) + 0.25;
        end
        if s2 ~= 3
            P_chain(grid_coord(s1,s2,3), grid_coord(s1,s2+1,3)) = 0.25;
        else
            P_chain(grid_coord(s1,s2,3), grid_coord(s1,s2,3)) = P_chain(grid_coord(s1,s2,3), grid_coord(s1,s2,3)) + 0.25;
        end
        if s1 ~= 1
...
for i=1:size(labels, 1)
    L(grid_coord(labels(i,1), labels(i,2), 3)) = labels(i,3); 
end
...
wins = observationSequences > 9;
win_rate = sum(wins(:,len))/sequences
% run local:
tic
[T_est, E_est] = hmmtrain(observationSequences, T_est, E_est, 'Tolerance', 1e-6,'Maxiterations', 25000, 'Verbose',true);
save("3x3case_study_"+string(round(n_seed)))
timeVal = toc
% for i=1:100
%     [T_est, E_est] = hmmtrain(observationSequences, T_est, E_est, 'Tolerance', 1e-6, 'Maxiterations',25,'Verbose',true);
%     disp(T_est)
%     save("3x3case_study_"+string(round(n_seed)))
%     timeVal = toc
% end